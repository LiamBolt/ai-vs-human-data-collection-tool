...
export default function ProctorLoginPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.role === 'RATER') {
        setError('This account is a Rater account. Use /rater/login instead.')
        return
      }
      setAuth(data.access_token, data.role, data.display_code)
      navigate('/proctor/batches', { replace: true })
    },
    onError: () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6">
      <FormShell
        ...
        footer={
          <span className="block text-center text-xs text-text-disabled">
            Rater?{' '}
            <a
              href="/rater/login"
